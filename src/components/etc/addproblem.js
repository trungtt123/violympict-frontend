...
function AddProblem(props) {
    ...
    const handleEditTestCase = (test, index) => {
        var data = {
            ...
            score: localStorage.getItem('score') === null ? test.timeLimit : JSON.parse(localStorage.getItem('editscore'))
        }
                ...
                var elmTmp = data.result.map((problem, index) => {
                    ...
                    return <>
                        ...
                        <div className="btnControl mb-2">
                            {title}
                            <button className="btn btn-info mr-1" onClick={() => handleSelectProblem(problem)}>Chọn</button>
                            <button className="btn btn-success mr-1" onClick={() => handleUnSelectProblem(problem)}>Bỏ chọn</button>
                            <button className="btn btn-warning text-light mr-1" onClick={() => handleEditProblem(problem)}>Chỉnh sửa</button>
                            <button className="btn btn-danger" onClick={() => handleEditProblem(problem)}>Xóa</button>
                        </div>
                    </>
                })
    ...
    return <>
        ...
        <div className="create-exam">
            ...
            <h4 className="text-info ml-4">Tạo bài tập</h4>
